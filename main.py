...
if not os.path.exists(COOKIES_FILE):
    logger.warning("⚠️ Cookies file not found! Restricted downloads will fail.")

app = Client("universal_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)

# Global State
user_data = {} # To store user specific settings
active_downloads = {} # To manage tasks
...
# --- 5. HELPER FUNCTIONS (Professional) ---
...
# --- 7. DOWNLOADER LOGIC (Universal) ---

@app.on_message(filters.regex(r"http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+") | filters.command(["dl"]))
async def link_handler(client, message):
    ...
    btns = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎥 Video (Best MKV)", callback_data=f"dl|video|{req_id}"),
         InlineKeyboardButton("🎵 Audio Only", callback_data=f"dl|audio|{req_id}")],
...
async def process_download(client, callback):
    ...
    await callback.message.delete()
    status = await callback.message.reply("🔄 **Initializing Download...**")
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Metadata Injection
            await status.edit("🏷️ **Adding Metadata...**")
            # Using FFmpeg to add AUTHOR_NAME
            ...
            if action == "audio":
                await app.send_audio(
                    ...
                    thumb=final_thumb, performer=AUTHOR_NAME, caption=cap,
                    progress=progress_bar, progress_args=(status, start, "⬆️ **Uploading Audio...**")
                )
            else:
                await app.send_video(
                    callback.message.chat.id, video=fpath, caption=cap,
            ...
            await status.delete()
            await callback.message.reply_text("✅ **Download Complete!**")
